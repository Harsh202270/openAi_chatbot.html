# openAi_chatbot.html
You can easily make your own chatbot using this project.

**First sign up on the OpenAI website.**  
**It is free to create an account.**

## 🔑 How to Get Your OpenAI API Key
Follow these steps to create your OpenAI API key:

1. Open this link: https://platform.openai.com/api-keys  
2. Click **“Create new secret key”**  
3. Enter any name for your key  
4. Click **“Create secret key”**  
5. Your key will appear (something like: `sk-proj-*****`). Copy it and click **Done**  
6. Now open **openAi_chatbot.html**  
7. Go to **line 92** and paste your API key  
8. Save the file — your chatbot is ready to use! 😊😊😊
